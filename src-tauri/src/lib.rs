...
// via TrayIcon.getById to attach the menu and is the target of set_alert.
...
// The menu-bar attention mark (ADR-0013): swap the tray between two black+alpha
// templates — the plain branch glyph, or the same glyph with an exclamation baked
// in. on=true (≥1 Skill Behind) shows the alert variant; on=false the plain glyph.
// This replaces ADR-0005's count-as-tray-title; the exact counts live in the menu
// rows. include_image! embeds both PNGs at compile time, so a missing asset fails
// the build (the assets are generated by icons/render-tray-icons.sh).
//
// set_icon_with_as_template, not set_icon: on macOS set_icon resets the template
// flag to false (tray-icon 0.23 hardcodes is_template=false), so a swapped icon
// would stop inverting — black on a dark bar, near-invisible. The _with_as_template
// form re-asserts the template atomically (and dodges a double-render flicker); off
// macOS it falls back to set_icon, where template is a no-op anyway.
#[tauri::command]
fn set_alert(app: tauri::AppHandle, on: bool) -> Result<(), String> {
    let tray = app
        .tray_by_id(TRAY_ID)
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Seeds the plain (no-alert) base template (black + alpha), not
            // bundle.icon's full-colour app icon: drawn at menu-bar size and, as a
            // template (icon_as_template), inverted for light/dark bars. set_alert
            // swaps it for the alert variant once a cycle finds a Behind Skill
            // (ADR-0013). include_image! embeds it at compile time, so a missing
